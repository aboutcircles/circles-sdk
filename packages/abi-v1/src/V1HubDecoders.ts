...
export class V1HubDecoders {
  ...
  decode(callData: string): { name: string, inputs: inputTypes.V1HubFunctionInputs } {
    ...
    switch (<V1HubFunctionName>functionFragment.name) {
      case 'checkSendLimit':
        decoded = this.decodeCheckSendLimitInputs(callData);
        break;
      case 'inflate':
        decoded = this.decodeInflateInputs(callData);
        break;
      case 'issuanceByStep':
        decoded = this.decodeIssuanceByStepInputs(callData);
        break;
      case 'limits':
        decoded = this.decodeLimitsInputs(callData);
        break;
      case 'organizations':
        decoded = this.decodeOrganizationsInputs(callData);
        break;
      case 'pow':
        decoded = this.decodePowInputs(callData);
        break;
      case 'seen':
        decoded = this.decodeSeenInputs(callData);
        break;
      case 'tokenToUser':
        decoded = this.decodeTokenToUserInputs(callData);
        break;
      case 'transferThrough':
        decoded = this.decodeTransferThroughInputs(callData);
        break;
      case 'trust':
        decoded = this.decodeTrustInputs(callData);
        break;
      case 'userToToken':
        decoded = this.decodeUserToTokenInputs(callData);
        break;
      case 'validation':
        decoded = this.decodeValidationInputs(callData);
        break;
